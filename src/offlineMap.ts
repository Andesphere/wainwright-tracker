export const LAKE_DISTRICT_OFFLINE_BOUNDS = [-3.55, 54.25, -2.65, 54.8] as const
...
export const OPENTOPOMAP_TILE_TEMPLATE = 'https://tile.opentopomap.org/{z}/{x}/{y}.png'
...
function latitudeToTileY(latitude: number, zoom: number) {
  ...
  return Math.floor(((1 - Math.asinh(Math.tan(radians)) / Math.PI) / 2) * tiles)
}
...
export async function downloadLakeDistrictMap(
  ...
  const progress: DownloadProgress = { downloaded: 0, total: urls.length, failed: 0 }
  let nextIndex = 0
  ...
  if (progress.failed > 0) {
    throw new Error(`${progress.failed} map tiles failed to download. Try again to fill the gaps.`)
  }
